...
    •    Training loop: Post-session grading and scorecards via [grading_service.py](backend/app/services/grading_service.py) (separate non-streaming LLM call); emotion/objection state machine documented in [docs/specs/EMOTION_SIMULATION_ENGINE.md](docs/specs/EMOTION_SIMULATION_ENGINE.md).
...
    4    Deepgram
    ◦    [PRD_latency_optimization.md](docs/specs/PRD_latency_optimization.md) Task 1 (VAD + trigger_finalization) is already present in ws.py / DeepgramSttClient. 
...
    6    Post-turn / DB work
Defer non-critical persistence off the critical path (remaining items in [PRD_latency_optimization.md](docs/specs/PRD_latency_optimization.md): deferred commit, adaptive flush) so STT final → LLM call is unblocked.
    7    Long-term architecture
[docs/specs/architecture.md](docs/specs/architecture.md) mentions OpenAI Realtime API as an all-in-one voice stack. Worth a spike if you want single-hop voice and are willing to rework the client and state machine.
...
    3    Barge-in realism
[set_interrupt](backend/app/voice/ws.py) cancels TTS tasks; validate on device that client buffer flush matches PRD notes in [PRD_latency_optimization.md](docs/specs/PRD_latency_optimization.md) (no 200–500 ms ghost audio).
...
Rationale
A
Instrument TTFT, first MP3 byte, STT finalize lag
Confirms whether TTS buffering or LLM dominates
B
Client audio path for true streaming OR shorter first utterance
Largest perceived latency lever if logs show TTS dominates
C
Prompt caching + token budget enforcement
Cheap recurring win on LLM latency/cost
D
Deepgram endpointing + confidence tuning
Polish STT end-to-LLM handoff
E
Grading/orchestrator label alignment
Improves learning loop quality
No code changes in this message; this is a roadmap tied to your existing files and PRDs.
